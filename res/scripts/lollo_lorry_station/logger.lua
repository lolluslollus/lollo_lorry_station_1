...
return {
    ...
    warn = function(label, ...)
        ...
        print('lollo_lorry_station WARNING: ' .. label, ...)
    end,
    err = function(label, ...)
        if not(_isErrorLogActive) then return end
        print('lollo_lorry_station ERROR: ' .. label, ...)
    end,
    ...
    xpHandler = function(error)
        if not(_isExtendedLogActive) then return end
        print('lollo_lorry_station INFO:') debugPrint(error)
    end,
    xpWarningHandler = function(error)
        if not(_isWarningLogActive) then return end
        print('lollo_lorry_station WARNING:') debugPrint(error)
    end,
    xpErrorHandler = function(error)
        if not(_isErrorLogActive) then return end
        print('lollo_lorry_station ERROR:') debugPrint(error)
    end,
}
